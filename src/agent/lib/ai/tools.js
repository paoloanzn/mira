...
export const getUserTweets = tool({
  description: 'Get tweets from a specific Twitter user, limited to the most recent tweets.',
  parameters: z.object({
    username: z.string().min(1).describe('Twitter username to fetch tweets from'),
    maxTweets: z.number().optional().default(20).describe('Maximum number of tweets to fetch (max 100)')
  }),
...
export const getUserTweetsAndReplies = tool({
  description: 'Get tweets and replies from a specific Twitter user, limited to the most recent items.',
  parameters: z.object({
    username: z.string().min(1).describe('Twitter username to fetch tweets and replies from'),
    maxTweets: z.number().optional().default(20).describe('Maximum number of tweets to fetch (max 100)')
  }),
  execute: async ({ username, maxTweets }) => {
    const scraper = getScraper();
...
export const getLatestTweet = tool({
  ...
  parameters: z.object({
    username: z.string().min(1).describe('Twitter username to fetch the latest tweet from')
  }),
...
export const getUserRetweets = tool({
  description: 'Get retweets from a specific Twitter user, limited to the most recent retweets.',
  parameters: z.object({
    username: z.string().min(1).describe('Twitter username to fetch retweets from'),
    maxTweets: z.number().optional().default(20).describe('Maximum number of tweets to check for retweets (max 100)')
  }),
  execute: async ({ username, maxTweets }) => {
    const scraper = getScraper();
    const limit = Math.min(maxTweets, MAX_TWEETS_LIMIT);
...
export const getUserFollowers = tool({
  ...
  parameters: z.object({
    userId: z.string().min(1).describe('Twitter user ID to fetch followers from'),
    maxFollowers: z.number().optional().default(20).describe('Maximum number of followers to fetch (max 100)')
  }),
...
export const getUserFollowing = tool({
  ...
  parameters: z.object({
    userId: z.string().min(1).describe('Twitter user ID to fetch following from'),
    maxFollowing: z.number().optional().default(20).describe('Maximum number of following to fetch (max 100)')
  }),
...
export const searchTweets = tool({
  ...
  parameters: z.object({
    ...
    maxResults: z.number().optional().default(20).describe('Maximum number of results to fetch (max 50)'),
    mode: z.enum(['Latest', 'Top']).optional().default('Latest').describe('Search mode: Latest or Top')
  }),
...
export const searchProfiles = tool({
  ...
  parameters: z.object({
    ...
    maxResults: z.number().optional().default(10).describe('Maximum number of results to fetch (max 50)')
  }),
...
export const getProfile = tool({
  ...
  parameters: z.object({
    username: z.string().min(1).describe('Twitter username to fetch profile for')
  }),
  execute: async ({ username }) => {
    const scraper = getScraper();
