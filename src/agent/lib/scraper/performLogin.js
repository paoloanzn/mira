  ...
  try {
    ...
    if (credentials.cookies) {
      try {
        ...
        const cookieLoginSuccess = await withRetries(async () => scraper.isLoggedIn());
        if (cookieLoginSuccess) {
          return { success: true, message: 'Successfully logged in with cookies' };
        }
      } catch (error) {
        // If cookie login fails, fall back to password login
        console.warn('Cookie login failed, falling back to password login:', error.message);
      }
    }
    ...
    try {
      await withRetries(async () => 
        scraper.login(credentials.username, credentials.password, credentials.email)
      );
